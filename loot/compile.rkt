...
(define (compile-e e c t?)
  (match e
    ...
    [(Values es)
      ...
      [0 (seq
          (Mov 'r13 0)
          (Mov (Offset rbx 0) 'r13)
          (Mov rax type-values))]
      [1 (compile-e (first es) c t?)] ; case for one arg
      [_ (seq
        (Mov 'r13 (length es))  ; store the amount of values on the heap
        (Mov (Offset rbx 0) 'r13) 
        (Mov 'rcx rbx) ; store heap pointer
        (push-to-heap es c t?) ; push all values to heap
        (Mov rbx 'rcx) ; restore heap pointer
        (Mov rax type-values))]) ; mark rax as a values 
      ]
    [(Let-values xs e1 e2)      
       (let ((single (gensym))
             (none (gensym))
      ...
      (seq
      (compile-e e1 c t?)                 ; compile e1
      (Mov 'r12 (length xs))              ; retrieve the length of the names r12 - names r13 - args

       
      (Cmp rax type-values)               ; verify that the result of e1 has multiple values
      (Jne single)                        ; if it is not either we will bind a single value
        
      (Mov 'r13 (Offset rbx 0))            ; retrieve the length of arguments from the heap

      (Cmp 'r13 'r12)                      ; verify there are an equal number of names and values
      (Jne 'raise_error)                  ; if there is not raise an error

      

      (transfer-to-stack xs e2 c t?)    ; push all of the values onto the stack
      (move-names xs e2 c t?)              ; add all of the xs names to the environment
      (Jmp end)                           ; skip over the single

      (Label single)                      ; compile the case where we are only binding a single value
      (Mov 'r13 1)                        ; verify that there is only one name to bind
      (Cmp 'r12 'r13)
      (Jne 'raise_error)                  ; if there is not raise an error

      (Push rax)                          ; push the result of e1 on the stack
      (move-names xs e2 c t?)              ; add the name onto the environment

      (Label end)                     ; end label 
      ))]
    ))

  ; moves names onto the environment
  (define(move-names xs e2 c t?)
          (match xs
          [(cons x y) 
          (seq
          (move-names y e2 (cons x c) t?) ; move all names onto the stack
          (Add rsp 8))]                   ; increment the stack pointer
          [_ (compile-e e2 c t?)])
        )

  (define (push-to-heap es c t?)
  (match es
    [(cons e es)
      (seq (compile-e e c t?)     ; compile the current value
        (Mov (Offset rbx 8) rax) ; push the current value onto the heap
        (Add rbx 8)              ; grow the heap
        (push-to-heap es c t?))] ; push the rest
     ['() (seq)]))               ; done

(define(transfer-to-stack xs e2 c t?)
    (let ((loop (gensym))
          (end (gensym)))
          (seq
          (Label loop)                      ; while length xs > 0
          (Cmp 'r13 0)                      
          (Je end) 
          (Mov 'r11 (Offset rbx 8))         ; grab value off heap
          (Add rbx 8)                       ; expand heap
          (Sub 'r13 1)                      ; dec length
          (Push 'r11)                       ; push it to stack
          (Jmp loop)
          (Label end)
          ))
        )
