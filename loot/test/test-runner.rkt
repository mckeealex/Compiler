...
(define (test-runner run)
   ;; Values tests
   (check-equal? (run '(values 17)) 17)
   (check-equal? (run '(values "x")) (values "x"))
   (check-equal? (run '(values 0)) (first (list 0 0)))
   (check-equal? (run '(values #f)) #f)

   (define-values (x y) (run '(values 1 2)))
   (check-equal? x 1) 
   (check-equal? y 2)

   (check-equal? (run '(add1 (values 1))) 2) 
   (check-equal? (run '(add1 (values))) 'err) 

   (define-values (a b) (run '(values (+ 4 1) (if #f 6 1))))
   (check-equal? a 5) 
   (check-equal? b 1)

   (define-values (c d) (run '(values (- 70 1) (+ 10 410))))
   (check-equal? c 69) 
   (check-equal? d 420)
  
   
   (check-equal? (run '(add1 (values 1 2))) 'err) 
   
   

   ; Let-Values tests
  (check-equal? (run '(let-values ([(x y) (values 1 2)]) (+ x y))) 3)
  (check-equal? (run '(let-values ([(x) (add1 0)]) (add1 x)))2)
  (check-equal? (run '(let-values ([(x y z) (values 1 2 3)]) (+ (+ x y) z))) 6)
  (check-equal? (run '(let-values ([(x y z) (values 1 2 3)]) (+ x y))) 3)
  (check-equal? (run '(let-values ([(x y z) (values 1 2 3)]) (+ x z))) 4)
  (check-equal? (run '(let-values ([(x) (add1 0)]) (+ (add1 x) (values (+ 1 2)))))5)

   (check-equal? (run '(let-values ([(x y z) (values 1 2)]) 0))'err)
   (check-equal? (run '(let-values ([(x y z) (values 1 2 3 2)]) 0))'err)
   (check-equal? (run '(let-values ([() (values)]) (+ 5 7))) 12)
   (check-equal? (run '(let-values ([(x) (values)]) (+ 5 7))) 'err)
   (check-equal? (run '(let-values ([(x y) 2]) x)) 'err)
   (check-equal? (run '(let-values ([(x y) (values 3)]) x)) 'err)
   (check-equal? (run '(let-values ([(x) 2]) x)) 2)
